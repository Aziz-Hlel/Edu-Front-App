...
const Del1_row = ({ name, id }: { name: string; id: number; }) => {
    ...
    return (
        <>
            ...
            {/* Spacing row */}
            <tr className=" 2xl:h-16 h-11 sticky top-32  bg-transparent  z-10" >
                <td />
                <td colSpan={5} />
            </tr>

            {/* Del row */}
            <tr className=" bg-transparent  2xl:h-20 h-16 shadow-xl sticky top-32 z-10 shadow-transparent" onClick={() => set_collapse((prev) => !prev)}>

                {/* DelName td */}
                <td colSpan={6} className="bg-blue-600 pr-4 text-white font-medcium text-right 2xl:text-2xl text-xl  rounded-r-xl  z-20 shadow-lg ">
                    {name}
                </td>
