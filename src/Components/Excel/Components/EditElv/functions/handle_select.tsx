...
export const handle_MouseDown_prev_ecole = async (event: React.MouseEvent<HTMLDivElement, MouseEvent>, set_eleve_function: Function) => {
    const prev_ecole_str_val = event.currentTarget.getAttribute('val-att');
    const prev_ecole_id_val = event.currentTarget.id;
    event.preventDefault(); // idk why ama hedhi lezmk tzidha bch l handle_focus t5dm

    set_eleve_function('prev_ecole', prev_ecole_str_val);
    await set_eleve_function('prev_ecole_id', prev_ecole_id_val);
