...
const UidInput_Component = () => {

    const { change_elv, } = useTransferredElvsContext();
    ...
    const getElv = async (uid: string) => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get(ApiGateway.excel.getStudent + "/" + uid, {
                    validateStatus: function (status) {
                        return status >= 200 && status < 300 || status === 404; // Treat 404 as a valid response
                    }
                });
                ...
                if (response.status === 404) {
                    set_loading(false)
                    // set_error_uid(true)
                    alertMessage("لم يتم العثور على المُعرّف", "error")

                    change_elv((_) => ({ id: Number(uid), name: "", fatherName: "", dob: "", level: "", prev_ecole: "", prev_ecole_id: 0, Del1: "", Del1_id: 0, next_ecole: "", next_ecole_id: 0, reason: "تغيير مقر الإقامة", decision_id: !isAdmin ? -1 : 0, decision: !isAdmin ? "   " : "مع الموافقة", comments: "" }))

                    handle_focus("manual");
                    return
                }

                const jsonData = await (response.data.data as any);
                
                change_elv((_) => ({ ...jsonData,prev_ecole: jsonData.schoolName, prev_ecole_id: jsonData.school_id, level: "", Del1: "", Del1_id: 0, next_ecole: "", next_ecole_id: 0, reason: "تغيير مقر الإقامة", decision_id: !isAdmin ? -1 : 0, decision: !isAdmin ? "    " : "مع الموافقة", comments: "" }))
                ...
                <div className=" ml-5">
                    <div className="flex flex-col">
                        ...
                        <div className="flex" dir='rtl'>
                            ...
                            <span className="text-lg h-10  ml-4  flex items-center">المعرف : </span>
